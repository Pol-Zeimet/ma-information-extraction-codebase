...
class DataGenerator(keras.utils.Sequence):
    def __init__(self, graph_src, label_src, labels, doc_names,additional_data_src, batch_size=32, node_count=512, node_shape=768,
                 edge_count=40000, edge_shape=5, shuffle=True, one_hot=False):
        # Initialization
        self.graph_src = graph_src
        self.label_src = label_src
        self.batch_size = batch_size
        self.list_doc_names = doc_names
    ...
    def __len__(self):
        ...
        return int(np.floor(len(self.list_doc_names) / self.batch_size))
    ...
    def __getitem__(self, index):
        ...
        # Find list of doc_names
        list_doc_names_temp = [self.list_doc_names[k] for k in indexes]

        # Generate data
        X, y = self.__data_generation(list_doc_names_temp)
    ...
    def get_tokens_and_positions(self, index):
        ...
        for doc_name in [self.list_doc_names[k] for k in indexes]:
            subset = self.additional_data_df[self.additional_data_df.doc_name == doc_name]
    ...
    def on_epoch_end(self):
        ...
        self.index_list = np.arange(len(self.list_doc_names))
        if self.shuffle is True:
            np.random.shuffle(self.index_list)

    def __data_generation(self, list_doc_names_temp):
        ...
        # Generate data
        for i, doc_name in enumerate(list_doc_names_temp):
            try:
                # laden Graph
                graph = np.load(os.path.join(self.graph_src, doc_name + '.npy'), allow_pickle=True)
                ...
                # load Labels
                i_labels = np.load(os.path.join(self.label_src, doc_name + '.npy'), allow_pickle=True)
                ...
                print('error with file: ' + doc_name)
                print(error)
                self.failed_loads.append(doc_name)
